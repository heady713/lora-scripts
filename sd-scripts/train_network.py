...
def train(args):
    ...
    def save_model(ckpt_name, unwrapped_nw, steps, epoch_no, force_sync_upload=False):
        # raise Exception(ckpt_name,unwrapped_nw, steps, epoch_no)
            ...
            if accelerator.sync_gradients:
                ...
                if args.save_every_n_steps is not None and global_step % args.save_every_n_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        print(f"current_loss:{loss_total / len(loss_list)}, best_loss:{best_loss}")
                        ckpt_name = train_util.get_last_ckpt_name(args, "." + args.save_model_as)
                        if loss_total / len(loss_list) < best_loss:
                            save_model(ckpt_name, unwrap_model(network), global_step, epoch)
                            ...
                            best_loss = loss_total / len(loss_list)
    ...
    if is_main_process:
        ...
        if loss_total / len(loss_list) < best_loss:
            save_model(ckpt_name, network, global_step, num_train_epochs, force_sync_upload=True)
            best_loss = loss_total / len(loss_list)
        print(f"best loss: {best_loss}, ckpt_name: {ckpt_name}")
        print("model saved.")
